...
const els = {
  ...
  curtailmentVariance: document.getElementById("curtailmentVariance"),
  curtailmentVarianceSub: document.getElementById("curtailmentVarianceSub"),
...
const chartLabelFont = "12px Aptos, 'Segoe UI Variable', 'Segoe UI', Arial, sans-serif";
const chartSmallFont = "11px Bahnschrift, 'DIN Alternate', Aptos, 'Segoe UI Variable', 'Segoe UI', Arial, sans-serif";
...
const metricConfig = {
  ...
  curtailment: {
    label: "Curtailment",
    unit: "MWh",
    actual: "actual_curtailment",
    planKey: "curtailment",
    format: (value) => number(value, 0),
    adversePositive: true,
  },
};
...
fetch(`data/settlement-dashboard-data.json?ts=${Date.now()}`, { cache: "no-store" })
  .then((response) => {
...
function initialize() {
  const savedTheme = localStorage.getItem("moleculeSettlementDashboardTheme");
  if (savedTheme === "sbe" || savedTheme === "current") state.theme = savedTheme;
  state.filtersCollapsed = localStorage.getItem("moleculeSettlementDashboardFiltersCollapsed") === "true";
  const scenarios = planScenarios();
  const savedPlan = localStorage.getItem("moleculeSettlementDashboardPlanScenario");
  ...
  els.planButtons.forEach((button) => {
    ...
    button.addEventListener("click", () => {
      ...
      localStorage.setItem("moleculeSettlementDashboardPlanScenario", state.planScenario);
      render();
    });
  });
  ...
  els.themeButtons.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      localStorage.setItem("moleculeSettlementDashboardTheme", state.theme);
      applyTheme();
      render();
    });
  });
  ...
  els.toggleFilters.addEventListener("click", () => {
    ...
    localStorage.setItem("moleculeSettlementDashboardFiltersCollapsed", String(state.filtersCollapsed));
...
function renderHeader(rows) {
  ...
  updatePlanStatus();
  els.heroSub.textContent = `${selectionLabel()} | ${projectCount} settlement reports | ${dayCount} actual days | ${state.start} to ${state.end} | ${state.planScenario} Plan`;
}
...
function dataQualityMessages() {
  ...
  const wprRows = state.data.wpr_rows || [];
  ...
  const missingWpr = wprRows.length ? missingProjectDates(dates, projects, wprKeys) : [];
  const messages = [];
  if (missingActuals.length) {
    messages.push(`Molecule settlement actuals may be missing for ${formatMissingProjectDates(missingActuals)}.`);
  }
...
function renderKpis(totals) {
  ...
  const curtailmentVariance = totals.curtailment.aopCount ? totals.curtailment.actualCovered - totals.curtailment.aop : null;
  const curtailmentPct = totals.curtailment.aop ? curtailmentVariance / totals.curtailment.aop : null;
  ...
  els.curtailmentVariance.textContent = totals.curtailment.actualCount ? `${number(totals.curtailment.actual, 0)} MWh` : "n/a";
  els.curtailmentVarianceSub.textContent = curtailmentVariance === null
    ? "No modeled curtailment"
    : `Modeled ${number(totals.curtailment.aop, 0)} MWh | Var ${signedNumber(curtailmentVariance, 0)} MWh`;
  toneMetric(els.curtailmentVariance, curtailmentVariance, metricConfig.curtailment);
}
...
function renderWprCards(rows) {
  ...
  const cards = projects.map((project) => {
    ...
    const validRows = projectRows.filter((row) => row.wpr_percent !== null && row.wpr_percent !== undefined);
    const latest = projectRows.at(-1);
    const rangeExpected = projectRows.reduce((sum, row) => sum + (row.wpr_expected_energy || 0), 0);
    const rangeModeled = projectRows.reduce((sum, row) => sum + (row.wpr_modeled_energy || 0), 0);
    const rangeWpr = rangeModeled ? 100 * rangeExpected / rangeModeled : null;
    const rangeCurtailment = projectRows.reduce((sum, row) => sum + (row.curtailed_energy || 0), 0);
    const value = latest?.wpr_dashboard ?? latest?.wpr_percent ?? null;
    const width = value === null ? 0 : Math.max(0, Math.min(130, value));
    const subtitle = latest && value !== null
      ? `${latest.date} | range ${formatWpr(rangeWpr)} | curtailed ${number(rangeCurtailment, 0)} MWh`
      : latest
        ? `${latest.date} | ${wprMissingReason(latest)} | curtailed ${number(rangeCurtailment, 0)} MWh`
...
function wprMissingReason(row) {
  const missing = [];
  if (row.wpr_expected_energy === null || row.wpr_expected_energy === undefined) missing.push("expected energy");
  if (row.wpr_modeled_energy === null || row.wpr_modeled_energy === undefined || row.wpr_modeled_energy === 0) {
    missing.push("modeled energy");
  }
  return missing.length ? `Bazefield missing ${missing.join(" and ")}` : "WPR unavailable from Bazefield";
}
...
function renderTable(rows) {
  const { isProjectDetail, sorted, sortLabel, sortDirection } = currentTableGroups(rows);
  const columns = tableColumns(isProjectDetail);
  els.tableTitle.textContent = isProjectDetail ? `${selectionLabel()} ${grainLabel(state.grain)} View` : `${selectionLabel()} Performance`;
  els.tableSub.textContent = `${sorted.length} rows | ${state.start} to ${state.end} | Sorted by ${sortLabel} ${sortDirection === "asc" ? "ascending" : "descending"}`;
  els.tableHeader.innerHTML = columns
    .map((column) => {
      const active = activeTableSortKey(isProjectDetail) === column.key;
      const ariaSort = active ? (sortDirection === "asc" ? "ascending" : "descending") : "none";
      return `<th aria-sort="${ariaSort}">
        <button class="table-sort" type="button" data-table-sort="${column.key}">
          <span>${escapeHtml(column.label)}</span>
          <i aria-hidden="true">${active ? (sortDirection === "asc" ? "ASC" : "DESC") : ""}</i>
        </button>
      </th>`;
    })
    .join("");
  els.tableBody.innerHTML = sorted
    .map((group) => {
      ...
      const curtailVar = metricVariance(group, metricConfig.curtailment);
      ...
      return `<tr data-project="${isProjectDetail ? "" : escapeHtml(group.label)}" data-start="${group.start}" data-end="${group.end}">
        ...
        <td class="${varianceClassForMetric(revVar, metricConfig.revenue)}">${revVar === null ? "n/a" : currencySigned(revVar)}</td>
        ...
        <td class="${varianceClassForMetric(genVar, metricConfig.generation)}">${genVar === null ? "n/a" : signedNumber(genVar, 0)}</td>
        <td class="${varianceClass(genVarPct)}">${variancePerformanceText(genVarPct)}</td>
        <td>${group.actualCurtailmentCount ? number(group.actual_curtailment, 0) : "n/a"}</td>
        <td>${group.curtailmentPlanCount ? number(group.plan_curtailment, 0) : "n/a"}</td>
        <td class="${varianceClassForMetric(curtailVar, metricConfig.curtailment)}">${curtailVar === null ? "n/a" : signedNumber(curtailVar, 0)}</td>
        <td>${group.days}</td>
      </tr>`;
    })
    .join("");
  [...els.tableHeader.querySelectorAll("[data-table-sort]")].forEach((button) => {
    button.addEventListener("click", () => setTableSort(button.dataset.tableSort, isProjectDetail));
  });
...
function currentTableGroups(rows) {
  ...
  const sort = normalizedTableSort(isProjectDetail, activeMetric);
  const sorted = sortTableGroups(groups, sort.key, sort.direction);
  const column = tableColumns(isProjectDetail).find((item) => item.key === sort.key);
  return { isProjectDetail, sorted, sortLabel: column?.label || "Value", sortDirection: sort.direction };
}

function tableColumns(isProjectDetail) {
  return [
    { key: "label", label: isProjectDetail ? "Period" : "Project", type: "text" },
    { key: "actual_revenue", label: "Actual Rev", type: "number" },
    { key: "aop_revenue", label: `${state.planScenario} Rev`, type: "number" },
    { key: "revenue_var", label: "Rev Var", type: "number" },
    { key: "revenue_perf", label: "Rev Var / Perf", type: "number" },
    { key: "actual_generation", label: "Actual Gen", type: "number" },
    { key: "aop_generation", label: `${state.planScenario} Gen`, type: "number" },
    { key: "generation_var", label: "Gen Var", type: "number" },
    { key: "generation_perf", label: "Gen Var / Perf", type: "number" },
    { key: "actual_curtailment", label: "Actual Curtail", type: "number" },
    { key: "plan_curtailment", label: `${state.planScenario} Modeled Curtail`, type: "number" },
    { key: "curtailment_var", label: "Curtail Var", type: "number" },
    { key: "days", label: "Days", type: "number" },
  ];
}

function normalizedTableSort(isProjectDetail, activeMetric) {
  if (state.tableSort && tableColumns(isProjectDetail).some((column) => column.key === state.tableSort.key)) {
    return state.tableSort;
  }
  return {
    key: isProjectDetail ? "label" : activeMetric.actual,
    direction: "desc",
  };
}

function activeTableSortKey(isProjectDetail) {
  return normalizedTableSort(isProjectDetail, metricConfig[state.metric]).key;
}

function setTableSort(key, isProjectDetail) {
  const current = normalizedTableSort(isProjectDetail, metricConfig[state.metric]);
  const column = tableColumns(isProjectDetail).find((item) => item.key === key);
...
function sortTableGroups(groups, key, direction) {
  const multiplier = direction === "asc" ? 1 : -1;
  return groups.sort((a, b) => {
    const left = tableSortValue(a, key);
    const right = tableSortValue(b, key);
    if (typeof left === "string" || typeof right === "string") {
      const textCompare = String(left || "").localeCompare(String(right || ""), undefined, { numeric: true });
    ...
    if (leftMissing && rightMissing) return a.label.localeCompare(b.label, undefined, { numeric: true });
    if (leftMissing) return 1;
    if (rightMissing) return -1;
    const numericCompare = left === right ? a.label.localeCompare(b.label, undefined, { numeric: true }) : left - right;
    return numericCompare * multiplier;
  });
}

function tableSortValue(group, key) {
  if (key === "label") return group.label;
  if (key === "actual_generation") return group.actual_generation;
  if (key === "aop_generation") return group.generationAopCount ? group.aop_generation : null;
  if (key === "generation_var") return metricVariance(group, metricConfig.generation);
  if (key === "generation_perf") return metricPerformancePct(groupMetricVariancePct(group, metricConfig.generation));
  if (key === "actual_curtailment") return group.actualCurtailmentCount ? group.actual_curtailment : null;
  if (key === "plan_curtailment") return group.curtailmentPlanCount ? group.plan_curtailment : null;
  if (key === "curtailment_var") return metricVariance(group, metricConfig.curtailment);
  if (key === "actual_revenue") return group.actual_revenue;
  if (key === "aop_revenue") return group.revenueAopCount ? group.aop_revenue : null;
  if (key === "revenue_var") return metricVariance(group, metricConfig.revenue);
  if (key === "revenue_perf") return metricPerformancePct(groupMetricVariancePct(group, metricConfig.revenue));
  if (key === "days") return group.days;
  return null;
}
...
function exportCurrentCsv() {
  ...
  const headers = [
    ...
    "Actual Curtailed MWh",
    `${state.planScenario} Modeled Curtailment MWh`,
    "Curtailment Variance MWh",
  ...
  const csvRows = sorted.map((row) => {
    ...
    const planCurtailment = rowPlanValue(row, "curtailment");
    ...
    const curtailVar = planCurtailment === null || row.actual_curtailment === null || row.actual_curtailment === undefined
      ? null
      : row.actual_curtailment - planCurtailment;
    const curtailVarPct = planCurtailment ? curtailVar / planCurtailment : null;
    ...
    return [
      ...
      csvNumber(row.actual_curtailment, 3),
      csvNumber(planCurtailment, 3),
      csvNumber(curtailVar, 3),
      csvPercent(curtailVarPct),
  ...
  const filename = `sb-energy-settlement-daily-project-${safeFilePart(state.planScenario)}-${state.start}-to-${state.end}.csv`;
  ...
  rows.forEach((row) => {
    ...
    if (!map.has(period.key)) {
      map.set(period.key, {
        ...
        aop_revenue: 0,
        revenueAopCount: 0,
        actual_curtailment: 0,
        actual_curtailment_covered: 0,
        plan_curtailment: 0,
        curtailmentPlanCount: 0,
        actualCurtailmentCount: 0,
        rowCount: 0,
        dates: new Set(),
      });
    }
    ...
    if (row.actual_curtailment !== null && row.actual_curtailment !== undefined) {
      group.actual_curtailment += row.actual_curtailment || 0;
      group.actualCurtailmentCount += 1;
    }
    group.rowCount += 1;
    ...
    if (planCurtailment !== null) {
      group.plan_curtailment += planCurtailment;
      group.actual_curtailment_covered += row.actual_curtailment || 0;
      group.curtailmentPlanCount += 1;
    }
  });
...
function sumRows(rows) {
  ...
  return {
    ...
    curtailment: {
      actual: grouped.actual_curtailment,
      actualCovered: grouped.actual_curtailment_covered,
      aop: grouped.plan_curtailment,
      aopCount: grouped.curtailmentPlanCount,
      actualCount: grouped.actualCurtailmentCount,
      coverage: grouped.rowCount ? grouped.curtailmentPlanCount / grouped.rowCount : 0,
    },
  };
}
...
function drawVarianceChart(canvas, groups, metric) {
  ...
  groups.forEach((group, index) => {
    ...
    if (variance !== null) {
      ...
      ctx.fillStyle = varianceColor(variance, metric);
...
function rowPlanValue(row, planKey) {
  ...
  const legacyKey = planKey === "generation" ? "aop_generation" : planKey === "revenue" ? "aop_revenue" : "plan_curtailment";
...
function metricAop(group, metric) {
  ...
  if (metric.planKey === "curtailment") return group.curtailmentPlanCount ? group.plan_curtailment : null;
...
function metricVariance(group, metric) {
  ...
  if (metric.planKey === "curtailment") {
    return group.curtailmentPlanCount ? group.actual_curtailment_covered - group.plan_curtailment : null;
  }
...
function groupMetricVariancePct(group, metric) {
  ...
  if (metric.planKey === "curtailment") {
    return group.curtailmentPlanCount && group.plan_curtailment
      ? (group.actual_curtailment_covered - group.plan_curtailment) / group.plan_curtailment
      : null;
  }
...
function updateFilterVisibility() {
  ...
  els.toggleFilters.textContent = state.filtersCollapsed ? "Show Filters" : "Hide Filters";
  els.toggleFilters.setAttribute("aria-expanded", String(!state.filtersCollapsed));
}

function updateFilterSummary() {
  const range = state.start === state.end ? state.start : `${state.start} to ${state.end}`;
  els.filterSummary.textContent = `${range} | ${selectionLabel()} | ${state.planScenario} Plan | ${grainLabel(state.grain)}`;
}
...
function tooltipHtml(title, metric, actual, aop, variance) {
  const varianceClassName = varianceClassForMetric(variance, metric);
...
function varianceClassForMetric(value, metric) {
  if (value === null || value === undefined) return "muted";
  if (metric?.adversePositive) return value > 0 ? "negative" : value < 0 ? "positive" : "muted";
  return value >= 0 ? "positive" : "negative";
}

function varianceColor(value, metric) {
  const className = varianceClassForMetric(value, metric);
  if (className === "positive") return colors.positive;
  if (className === "negative") return colors.negative;
  return colors.text;
}
...
function toneMetric(element, value, metric) {
  element.classList.remove("positive", "negative", "muted");
  element.classList.add(varianceClassForMetric(value, metric));
}
